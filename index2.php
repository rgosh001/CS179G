<?php
  ...
  <body>
    ...
    <script type="text/javascript" language="javascript" src="popDoctorRatio.json"></script>
        ...
        geocoder.geocode({'address': address}, function(results, status) {
            if (status === 'OK' && address != "") {
                ...
                $(document).ready(function() {
                    $.getJSON('popDoctorRatio.json', function(json1) {
                        var found = 0;
                    ...
                    heatmapData = [];
                    $.getJSON('popDoctorRatioHospital.json', function(json1) {
                        json1.map((item) => {
                            item = JSON.parse(item)
                            if(address == item.ZipCode){
                                var marker = new google.maps.Marker({
                                    map: resultsMap,
                                    zoom: 3,
                                ...
                                var details = '<div id="content">'+
                                                ...
                                                '<h3 id="firstHeading" class="firstHeading">Provider Number: ' + item.ProviderNumber + '</h3>' +
                                                '<div id="bodyContent">'+
                                                '<p><b>Estimated Population: </b>' + (item.Pop/item.NumberOfHospitals) + '<br>' +
                                                '<b>Number of Doctors: </b>' + item.NumberOfDoctors + '<br>' +
                                                '<b>Ratio: </b>' + item.Ratio + '<br>';
                                
                                bindInfoWindow(marker, resultsMap, infowindow, details);

                                heatmapData.push( {location: new google.maps.LatLng(item.Y, item.X), weight: item.Ratio} );
                            }
                        });
